...
namespace ConsoleApplication.Comp
{
    public class Logs
    {
        ...
        public static void Info(string title, string content, bool isAppend, bool isError)
        {
            string strNow = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
            if (isError)
            {
                FileObj.WriteFile(title, string.Format("*************************ErrorHappens****************************{0}", Environment.NewLine), isAppend);
            }
            FileObj.WriteFile(title, string.Format("{0}\t{1}{2}", content, strNow, Environment.NewLine), isAppend);
            if (isError)
            {
                FileObj.WriteFile(title, string.Format("*****************************************************************{0}", Environment.NewLine), isAppend);
            }
        }

        public static void Info(string content, bool isAppend, bool isError)
        {
            string title = string.Format("{0}\\{1}", AppDomain.CurrentDomain.BaseDirectory, string.Format("logs_{0}.txt", DateTime.Now.ToString("yyyy-MM-dd")));
            Info(title, content, isAppend, isError);
        }

        public static void Info(string content)
        {
            Info(content, true, false);
        }

        public static void Info(string title, string content)
        {
            Info(title, content, true, false);
        }


        public static void WriteLogId(int logId, string title)
        {
            FileObj.WriteFile(string.Format("{0}\\{1}", AppDomain.CurrentDomain.BaseDirectory, title), logId.ToString(), false);
        }

        public static int ReadInfo(string title)
        {
            int i;
            int.TryParse(FileObj.ReadFile(string.Format("{0}\\{1}", AppDomain.CurrentDomain.BaseDirectory, title)), out i);
            return i;
        }
    }
